...
def detect_default_workspace(root: Path) -> Path | None:
    if root.as_posix().startswith("/mnt/"):
        return Path("/tmp/steamidtools-smx-build")
    return None
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compile SteamIDTools SourceMod plugin into a local build directory.")
    ...
    artifact_manifest = manifest.get("artifact", {}).get("sourcemod", {})
